    ...
    case "$action" in
        install)
            ...
            if [ -n "$specific_module" ]; then
                ...
                if is_valid_module "$specific_module"; then
                    execute_stage "grovel" "$specific_module"
                    execute_stage "install" "$specific_module"
                    execute_stage "verify" "$specific_module"
                else
                    log "ERROR" "Invalid module: $specific_module"
                    log "ERROR" "Available modules: $(get_enabled_modules "$CONFIG_FILE" | tr '\n' ' ')"
                    exit 1
                fi
            else
                # Install all modules
...
# Helper function to check if a module is valid
is_valid_module() {
    local module_name=$1
    local valid_modules=($(get_enabled_modules "$CONFIG_FILE"))
    
    for valid_module in "${valid_modules[@]}"; do
        if [[ "$valid_module" == "$module_name" ]]; then
            return 0
        fi
    done
    return 1
...
# Update execute_stage to handle specific module
execute_stage() {
    local stage=$1
    local specific_module=$2
    local -a modules
